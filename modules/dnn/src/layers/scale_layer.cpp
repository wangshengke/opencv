...
public:
    ...
    )
    {
        ...
        const auto numParams = weights.size() ? weights.size() : bias.size();
        ...
        std::size_t inner_size = total(inShape, axis, -1);
        ...
        if (hasWeights && hasBias)
            ...
            csl::kernels::add_with_expansion<float>(stream, output, input, inner_size, weights);
    }
    ...
    )
    {
        ...
        if (hasWeights)
        {
            ...
            copyMatToTensor(weightsTensor, blobs[0], stream);
        }
        ...
        if (hasBias)
        {
            ...
            copyMatToTensor(biasTensor, blobs.back(), stream);
        }
    }
